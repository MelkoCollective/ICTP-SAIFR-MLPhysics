...
class RBM(object):
    ...
    # maximum number of hidden units if we want to calculate log of the partition function exactly.
    max_feasible_for_log_pf = 20
    ...
    def __init__(self, num_hidden, num_visible, num_samples=128, weights=None, visible_bias=None, hidden_bias=None):
        ...
        # visible bias
        default = tf.random_normal(shape=(self.num_visible, 1), mean=-0.0, stddev=0.01)
        #default = tf.zeros(shape=(self.num_visible, 1))
        self.visible_bias = self._create_parameter_variable(visible_bias, default)
        # hidden bias
        default = tf.random_normal(shape=(self.num_hidden, 1), mean=-0.0, stddev=0.01)
        #default = tf.zeros(shape=(self.num_hidden, 1))
        self.hidden_bias = self._create_parameter_variable(hidden_bias, default)
        # pairwise weights
        default = tf.random_normal(shape=(self.num_visible, self.num_hidden), mean=0, stddev=0.01)
